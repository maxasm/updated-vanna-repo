...
echo ""
echo "# 3. Simple chat query (with conversation_id isolation)"
echo "curl -s -X POST $BASE_URL/api/v1/chat -H \"Content-Type: application/json\" -d '{\"message\": \"How many customers are there?\", \"headers\": {\"x-user-id\": \"test\", \"x-username\": \"tester\", \"x-user-groups\": \"api_users\", \"x-conversation-id\": \"sales\"}, \"metadata\": {\"test\": true}}'"
echo ""
echo "# 4. Table listing (same user, different conversation_id)"
echo "curl -s -X POST $BASE_URL/api/v1/chat -H \"Content-Type: application/json\" -d '{\"message\": \"Show me all tables\", \"headers\": {\"x-user-id\": \"test\", \"x-username\": \"tester\", \"x-user-groups\": \"api_users\", \"x-conversation-id\": \"inventory\"}, \"metadata\": {\"test\": true}}'"
echo ""

echo "# 5. Fetch conversation history (scoped)"
echo "curl -s '$BASE_URL/api/v1/conversation/history?user_identifier=test&conversation_id=sales' | python3 -m json.tool"
echo ""
echo "# 6. With pretty JSON output"
