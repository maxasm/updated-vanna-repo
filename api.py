...
class EnhancedChatHandler:
    ...
    async def handle_chat_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Track CSV filename from agent text output
            ...
            async for component in self.agent.send_message(
                ...
                if hasattr(component, 'simple_component') and hasattr(component.simple_component, 'text'):
                    ...
                    # or "Saved to file: query_results_3d6ebedc.csv"
                    ...
                    if csv_match and not captured_csv_filename:
                        ...
                        logger.info(f"Captured CSV filename from agent text: {captured_csv_filename}")
                    ...
                    if not captured_csv_filename:
                        ...
                        if csv_file_match and 'query_results' in csv_file_match.group(1):
                            ...
                            logger.info(f"Captured CSV filename from generic pattern: {captured_csv_filename}")
                ...
                if captured_csv_path:
                    ...
                    if query_results_dir.exists():
                        ...
                        if potential_csv.exists():
                            ...
                            logger.info(f"Found captured CSV in query_results directory: {csv_path}")
                        else:
                            logger.warning(f"Captured CSV filename '{captured_csv_filename}' not found in query_results directory")
            ...
            if csv_path:
                ...
                logger.info(f"CSV file determined: {csv_path}")
    ...
    def _find_latest_csv(self, max_age_seconds: int = 30) -> Optional[str]:
        ...
        Args:
            max_age_seconds: Only return CSV files modified within this time window
                             to reduce race conditions with other queries
        """
        try:
            import time
...
def create_app() -> FastAPI:
    ...
    # Remove the base server's SSE and poll endpoints so our custom endpoints can take precedence
    # Find and remove the POST /api/vanna/v2/chat_sse and /api/vanna/v2/chat_poll routes from base server
    routes_to_remove = []
    for i, route in enumerate(app.routes):
        if hasattr(route, 'path') and route.path in ["/api/vanna/v2/chat_sse", "/api/vanna/v2/chat_poll"]:
            if hasattr(route, 'methods') and 'POST' in route.methods:
                routes_to_remove.append(i)
                logger.info(f"Found base server endpoint at index {i}: {route.path}, will remove it")
    ...
    for i in sorted(routes_to_remove, reverse=True):
        ...
        logger.info(f"Removed base server endpoint: {removed_route.path}")
